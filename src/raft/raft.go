...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if (rf.voteFor == -1 || rf.voteFor == args.Id) && rf.CompareLog(args) {
...
func randInRange(l, r int) int64 {
	return int64(l + rand.Int()%(r-l))
}
...
func (rf *Raft) resetElectionTimeout() {
	...
	rf.electionTimeout = time.Millisecond * time.Duration(randInRange(300, 450))
}
	...
	for rf.killed() == false {
		...
		if rf.state == Leader {
			for id, _ := range rf.peers {
				...
				args := &AppendEntriesArgs{Term: rf.term, Id: rf.me}
...
func (rf *Raft) ticker() {
	for rf.killed() == false {
		...
		if time.Since(rf.lastHeatBeatTime) > rf.electionTimeout {
			rf.startElection()
		}
		rf.mu.Unlock()
